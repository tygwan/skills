...
#### [tdd-mvp-planner](./tdd-mvp-planner/)
Architecture 문서를 TDD 기반의 실행 가능한 개발 계획(PLAN.md)과 작업 목록(TODO.md)으로 체계적으로 변환합니다. Codex 자동 검증을 통해 완전성, 실현 가능성, TDD 준수를 보장합니다.

**주요 기능:**
- 7-phase 워크플로우 (Context → PLAN → TODO)
- TDD 원칙 준수 (RED → GREEN → REFACTOR → COMMIT)
- Codex 자동 검증 및 품질 보증
- 의존성 그래프 및 리스크 분석
- 도메인 중립적 (모든 프로젝트 타입 지원)
...
프로젝트 아이디어, 기능 설계, 문제 해결을 위한 체계적인 브레인스토밍 세션을 진행합니다. 다각도 분석과 구조화된 의사결정을 지원합니다.
...
TDD(Test-Driven Development) 방법론을 엄격하게 적용합니다. RED → GREEN → REFACTOR 사이클을 강제하여 모든 코드가 테스트로 검증되도록 합니다.

**주요 기능:**
- 철저한 TDD 원칙 준수 (테스트 우선)
...
체계적인 디버깅 방법론을 적용하여 복잡한 버그를 효율적으로 해결합니다. 근본 원인 분석과 증거 기반 접근을 강조합니다.
...
AI 에이전트의 품질을 체계적으로 검증하기 위한 테스팅 프레임워크입니다. 기능 테스트, 성능 테스트, 신뢰성 테스트를 지원합니다.
...
시스템 아키텍처 전문가 페르소나로 장기적 관점에서 확장 가능하고 유지보수 가능한 아키텍처를 설계합니다.
...
백엔드 개발 전문가 페르소나로 안정적이고 확장 가능한 서버 사이드 시스템을 구축합니다.
...
보안 운영 전문가 페르소나로 시스템의 보안성과 운영 안정성을 책임집니다.
...
시스템 복원력을 높이기 위한 설계 패턴과 구현 가이드를 제공합니다. Circuit Breaker, Retry, Timeout 등의 패턴을 포함합니다.

**주요 기능:**
- Circuit Breaker 패턴 구현
- Retry 및 백오프 전략
- Timeout 및 Deadline 관리
- Bulkhead 격리 패턴
- 복원력 테스트 시나리오

#### [consensus-engine](./consensus-engine/)
다중 AI 에이전트 간의 합의 알고리즘을 구현합니다. 분산 의사결정과 충돌 해결을 지원합니다.
...
#### [crypto-agent-architect](./crypto-agent-architect/)
암호화폐 거래 에이전트 시스템의 아키텍처를 설계합니다. Binance API 통합, 리스크 관리, DeFi 연동을 포함합니다.
...
#### [crypto-risk-management](./crypto-risk-management/)
암호화폐 거래의 리스크를 평가하고 관리하는 전략을 제공합니다. 포지션 사이징, 손절매, 리스크 메트릭을 포함합니다.
...
다양한 암호화폐 거래 전략을 구현하고 백테스팅합니다. 기술적 분석, 시그널 생성, 전략 최적화를 지원합니다.
